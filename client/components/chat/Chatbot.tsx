...
const FAQ: Record<string, string> = {
  "how to book": "Go to Dashboard → Public tab → Search a lawyer → Book appointment. You'll get a calendar link and confirmation.",
  "mock test": "Open Dashboard → Student tab → Mock Tests. Select a category and start. Results are instant with explanations.",
  "lawyer earnings": "Open Dashboard → Lawyer tab → Earnings section. Charts show daily and monthly revenue with filters.",
  "news": "Every tab includes a Law News panel that fetches the latest headlines.",
  "sign in": "Use the avatar in the header to sign in or sign up with Clerk. Sessions persist automatically.",
};
...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: "m0", role: "assistant", content: "Hi! I’m your Legal Hub assistant. Ask me about features or general law info." },
  ]);
        ...
        {open && (
          ...
          >
            <div className="flex items-center justify-between p-3 border-b">
              <div className="flex items-center gap-2"><Bot className="h-4 w-4" /><span className="font-semibold">Legal Hub Assistant</span></div>
              <Button size="icon" variant="ghost" aria-label="Close chat" onClick={() => setOpen(false)}><X className="h-4 w-4" /></Button>
            </div>
            ...
            <form className="p-3 flex items-center gap-2" onSubmit={(e) => { e.preventDefault(); send(); }}>
              <Input placeholder="Ask about Legal Hub…" value={input} onChange={(e) => setInput(e.target.value)} />
