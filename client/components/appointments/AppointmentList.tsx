...
const sampleAppointments: Appointment[] = [
  ...
  },
  {
    id: "2",
    client: "Neha Gupta",
    lawyer: "Adv. Rohan Singh",
    title: "Bail consultation",
...
function googleQuickAdd(appt: Appointment) {
  const text = encodeURIComponent(
    `${appt.title} — ${appt.client} with ${appt.lawyer}`,
  );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Sync any appointment to Google Calendar.
        </CardDescription>
      </CardHeader>
          ...
          >
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                {format(new Date(a.start), "MMM d, h:mm a")} —{" "}
                {format(new Date(a.end), "h:mm a")} • {a.client}
              </div>
            </div>
            <Button size="sm" onClick={() => googleQuickAdd(a)}>
              Add to Google
            </Button>
          </div>
        ))}
